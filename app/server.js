...
app.get('/health',(req,res)=>res.json({ok:true,version:'6.2.0',rooms:rooms.size,questions:allQuestions().length,categories:categoryCounts(),packs:allPacks().length}));
app.get('/api/questions/meta',(req,res)=>res.json({version:'6.2.0',questions:allQuestions().length,categories:categoryCounts(),difficulties:['easy','medium','hard'],packs:allPacks()}));
...
// Admin/editor
app.get('/api/admin/status',(req,res)=>res.json({keyRequired:!!ADMIN_KEY,remoteAllowed:!!ADMIN_KEY,version:'6.2.0'}));
...
app.get('/api/admin/dashboard',requireAdmin,(req,res)=>{const games=readJson(GAMES_FILE,[]),metrics=questionMetrics(),qs=allQuestions();const items=qs.map(q=>({id:q.id,q:q.q,c:q.c,quality:questionQuality(q,metrics),plays:metrics[q.id]?.plays||0,accuracy:metrics[q.id]?.total?pct(metrics[q.id].correct,metrics[q.id].total):null}));res.json({version:'6.2.0',questions:qs.length,categories:Object.keys(categoryCounts()).length,games:games.length,profiles:hallOfFame().players,rooms:rooms.size,review:items.filter(x=>x.plays>=3&&x.quality<45).length,tooEasy:items.filter(x=>x.plays>=3&&x.accuracy>=90).length,tooHard:items.filter(x=>x.plays>=3&&x.accuracy<=20).length,topReview:items.filter(x=>x.plays>=3).sort((a,b)=>a.quality-b.quality).slice(0,12)});});
app.get('/api/admin/backup',requireAdmin,(req,res)=>{ensureDataDir();const files=[HIGHSCORE_FILE,CUSTOM_FILE,PACK_FILE,OVERRIDE_FILE,GAMES_FILE,LEAGUE_FILE,PLAN_FILE,TOURNAMENT_FILE,TITLES_FILE,QUESTION_METRICS_FILE];const data={format:'resequiz-backup',version:1,appVersion:'6.2.0',createdAt:new Date().toISOString(),files:{}};for(const f of files)if(fs.existsSync(f))data.files[path.basename(f)]=readJson(f,null);res.setHeader('Content-Disposition',`attachment; filename=resequiz-backup-${new Date().toISOString().slice(0,10)}.json`);res.json(data)});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Resequiz 6.2.0 listening on ${PORT}`));
