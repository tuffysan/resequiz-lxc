...
const {verificationStatus,isPlayable,auditBank,backupName}=require('./question-verification');

const VERSION='24.1.2',PORT=Number(process.env.PORT||3000),HOST=process.env.HOST||'0.0.0.0';
...
const questions=()=>readJson(QUESTIONS_FILE,[]),playableQuestions=()=>questions().filter(isPlayable),results=()=>readJson(RESULTS_FILE,[]),users=()=>readJson(USERS_FILE,[]),reports=()=>readJson(REPORTS_FILE,[]),duels=()=>readJson(DUELS_FILE,[]),settings=()=>readJson(SETTINGS_FILE,{}),childQuestions=()=>readJson(CHILD_QUESTIONS_FILE,[]);
...
function saveResult(candidate){const all=results();candidate.awards=awardsForResult(candidate,all);if(candidate.userId)addLeaguePoints(candidate.userId,Math.max(20,Math.round(resultPct(candidate)*2))+(resultPct(candidate)===100?100:0));all.push(candidate);const kept=all.slice(-10000);writeJsonAtomic(RESULTS_FILE,kept);indexResults(quizDb,[candidate]);return candidate}
...
function trainingQuestions(userId,count=10,category=''){
 let pool=playableQuestions();if(category)pool=pool.filter(q=>q.category===category);if(!quizDb)return shuffle(pool).slice(0,count);
 try{
  const dueRows=quizDb.prepare(`SELECT question_id FROM review_schedule WHERE user_id=? AND due_at<=? ${category?'AND category=?':''} ORDER BY due_at ASC LIMIT ?`).all(...(category?[userId,new Date().toISOString(),category,count]:[userId,new Date().toISOString(),count]));
  if(dueRows.length){const map=new Map(pool.map(q=>[q.id,q])),dueQ=dueRows.map(r=>map.get(r.question_id)).filter(Boolean);if(dueQ.length>=count)return dueQ.slice(0,count);pool=[...dueQ,...pool.filter(q=>!dueQ.some(x=>x.id===q.id))]}
...
function updateReviewSchedule(userId,q,correct){
 if(!quizDb||!userId||!q)return null;
 try{
  const factKey=safe(q.factKey||deriveFactKey(q))||q.id,now=new Date(),old=quizDb.prepare('SELECT * FROM review_schedule WHERE user_id=? AND fact_key=?').get(userId,factKey);
  let ease=Number(old?.ease)||2.3,streak=Number(old?.correct_streak)||0,interval=Number(old?.interval_days)||0;
  if(correct){streak++;ease=Math.min(3.2,ease+0.08);interval=streak===1?2:streak===2?7:Math.min(180,Math.max(3,interval||7)*ease)}
  else{streak=0;ease=Math.max(1.3,ease-0.25);interval=1}
  const due=new Date(now.getTime()+interval*86400000).toISOString();
  quizDb.prepare(`INSERT INTO review_schedule(user_id,fact_key,question_id,category,interval_days,ease,correct_streak,last_result,last_seen,due_at) VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(user_id,fact_key) DO UPDATE SET question_id=excluded.question_id,category=excluded.category,interval_days=excluded.interval_days,ease=excluded.ease,correct_streak=excluded.correct_streak,last_result=excluded.last_result,last_seen=excluded.last_seen,due_at=excluded.due_at`).run(userId,factKey,q.id,q.category||'',interval,ease,streak,correct?1:0,now.toISOString(),due);
  return {factKey,intervalDays:Math.round(interval*10)/10,ease:Math.round(ease*100)/100,correctStreak:streak,dueAt:due};
 }catch{return null}
}
function reviewStatus(userId){
 if(!quizDb||!userId)return {due:0,learning:0,mastered:0,nextDue:null};
 try{
  const now=new Date().toISOString();
  const due=quizDb.prepare('SELECT COUNT(*) n FROM review_schedule WHERE user_id=? AND due_at<=?').get(userId,now).n;
  const learning=quizDb.prepare('SELECT COUNT(*) n FROM review_schedule WHERE user_id=? AND correct_streak<3').get(userId).n;
  const mastered=quizDb.prepare('SELECT COUNT(*) n FROM review_schedule WHERE user_id=? AND correct_streak>=3').get(userId).n;
  const next=quizDb.prepare('SELECT MIN(due_at) due FROM review_schedule WHERE user_id=? AND due_at>?').get(userId,now)?.due||null;
  return {due:Number(due)||0,learning:Number(learning)||0,mastered:Number(mastered)||0,nextDue:next};
 }catch{return {due:0,learning:0,mastered:0,nextDue:null}}
}
function playerInsights(userId){
 const mastery=masteryFor(userId),review=reviewStatus(userId),weakest=[...mastery].sort((a,b)=>a.mastery-b.mastery||b.games-a.games).slice(0,3),strongest=[...mastery].sort((a,b)=>b.mastery-a.mastery||b.games-a.games).slice(0,3);
 let avgResponseMs=0,answers=0,accuracy=0;
 if(quizDb)try{const r=quizDb.prepare('SELECT COUNT(*) n,SUM(correct) right,AVG(response_ms) ms FROM question_history WHERE user_id=?').get(userId);answers=Number(r.n)||0;accuracy=answers?Math.round((Number(r.right)||0)*100/answers):0;avgResponseMs=Math.round(Number(r.ms)||0)}catch{}
 const recommended=review.due?'review':weakest.length?'weakest':'quick';
 return {ok:true,review,weakest,strongest,answers,accuracy,avgResponseMs,recommended,recommendedCategory:weakest[0]?.category||''};
}
...
function questionMetricMap(){const m=new Map();if(quizDb)try{for(const x of quizDb.prepare('SELECT * FROM question_metrics').all())m.set(String(x.question_id),x)}catch{}return m}
function dynamicDifficulty(q,metric){const shown=Number(metric?.times_shown)||0;if(shown<12)return q.difficulty||'medium';const rate=Number(metric.times_correct||0)/shown;return rate>=.75?'easy':rate>=.45?'medium':'hard'}
function knowledgeMap(userId){if(!quizDb||!userId)return [];try{const rows=quizDb.prepare(`SELECT category,COUNT(*) answers,SUM(correct) correct,AVG(response_ms) response_ms FROM question_history WHERE user_id=? GROUP BY category`).all(userId);return rows.map(r=>{const accuracy=Math.round(100*Number(r.correct||0)/Math.max(1,Number(r.answers||0))),confidence=Math.min(100,Math.round(accuracy*Math.min(1,Number(r.answers)/25))),rank=confidence>=90?'Mästare':confidence>=80?'Expert':confidence>=70?'Guld':confidence>=55?'Silver':confidence>=40?'Brons':'Nybörjare';return {category:r.category||'Blandat',answers:Number(r.answers),accuracy,confidence,rank,avgResponseMs:Math.round(Number(r.response_ms)||0)}}).sort((a,b)=>b.confidence-a.confidence)}catch{return []}}
function questionHealthV3(){const qs=questions(),mm=questionMetricMap(),rows=qs.map(q=>{const metric=mm.get(q.id)||{},quality=qualityScore(q,metric);return {id:q.id,question:cleanQuestionText(q.question),category:q.category,subtopic:inferSubtopic(q),factKey:q.factKey||deriveFactKey(q),storedDifficulty:q.difficulty,dynamicDifficulty:dynamicDifficulty(q,metric),...quality,timesShown:Number(metric.times_shown)||0,correctRate:Number(metric.times_shown)?Math.round(Number(metric.times_correct||0)*100/Number(metric.times_shown)):null,reported:Number(metric.reported)||0}});return {ok:true,total:rows.length,average:rows.length?Math.round(rows.reduce((a,x)=>a+x.score,0)/rows.length):100,excellent:rows.filter(x=>x.score>=90).length,review:rows.filter(x=>x.score<70).length,rows:rows.sort((a,b)=>a.score-b.score).slice(0,300)}}


function weekKey(date=new Date()){const d=new Date(Date.UTC(date.getUTCFullYear(),date.getUTCMonth(),date.getUTCDate()));const day=d.getUTCDay()||7;d.setUTCDate(d.getUTCDate()+4-day);const y0=new Date(Date.UTC(d.getUTCFullYear(),0,1));return `${d.getUTCFullYear()}-W${String(Math.ceil((((d-y0)/86400000)+1)/7)).padStart(2,'0')}`}
function leagueTier(xp){return xp>=3500?['Diamant','💎']:xp>=2000?['Guld','🥇']:xp>=900?['Silver','🥈']:['Brons','🥉']}
function leagueBoard(userId){const wk=weekKey(),us=users(),names=new Map(us.map(u=>[u.id,u.displayName]));const rows=quizDb?(()=>{try{return quizDb.prepare('SELECT user_id,xp,games FROM league_points WHERE week_key=? ORDER BY xp DESC,games DESC LIMIT 100').all(wk)}catch{return []}})():[];const board=rows.map((r,i)=>{const [tier,icon]=leagueTier(Number(r.xp)||0);return {rank:i+1,userId:r.user_id,name:names.get(r.user_id)||'Quizspelare',xp:Number(r.xp)||0,games:Number(r.games)||0,tier,icon}});const mine=board.find(x=>x.userId===userId)||null;return {ok:true,week:wk,board:board.slice(0,25),mine}}
function addLeaguePoints(userId,xp){if(!quizDb||!userId)return;try{quizDb.prepare(`INSERT INTO league_points(user_id,week_key,xp,games,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(user_id,week_key) DO UPDATE SET xp=xp+excluded.xp,games=games+1,updated_at=excluded.updated_at`).run(userId,weekKey(),Math.max(10,Math.round(xp||0)),1,new Date().toISOString())}catch{}}
...
function dailyQuestions(date=dayKey(),count=10){
 const active=playableQuestions();
 const pool=active.filter(q=>q.verified||q.verification||q.source||q.sourceUrl||q.source_url);
 const source=pool.length>=count?pool:active;
...
app.get('/api/users/insights',(req,res)=>{const u=userFromReq(req);if(!u)return res.status(401).json({ok:false,error:'Logga in för personliga insikter.'});res.json(playerInsights(u.id))});
app.get('/api/review/status',(req,res)=>{const u=userFromReq(req);if(!u)return res.status(401).json({ok:false,error:'Logga in för repetition.'});res.json({ok:true,...reviewStatus(u.id)})});

app.get('/api/users/knowledge-map',(req,res)=>{const u=userFromReq(req);if(!u)return res.status(401).json({ok:false,error:'Logga in för kunskapskartan.'});res.json({ok:true,rows:knowledgeMap(u.id)})});
app.get('/api/admin/quality/v3',requireAdmin,(req,res)=>res.json(questionHealthV3()));

app.get('/api/league',(req,res)=>{const u=userFromReq(req);res.json(leagueBoard(u?.id||null))});
...
app.get('/health',(req,res)=>{let schema=null;try{schema=quizDb?.prepare('SELECT version FROM schema_info LIMIT 1').get()?.version||null}catch{}res.json({ok:true,version:VERSION,rooms:rooms.size,questions:questions().length,database:!!quizDb,schema,status:'healthy'})});
...
app.get('/api/admin/verification',requireAdmin,(req,res)=>{
 try{
  const audit=auditBank(questions());
  const status=safe(req.query.status),category=safe(req.query.category),q=safe(req.query.q).toLocaleLowerCase('sv-SE');
  let rows=audit.rows;if(status)rows=rows.filter(x=>x.status===status);if(category)rows=rows.filter(x=>x.category===category);if(q)rows=rows.filter(x=>(`${x.question} ${x.category} ${x.factKey}`).toLocaleLowerCase('sv-SE').includes(q));
  const offset=Math.max(0,Number(req.query.offset)||0),limit=Math.min(500,Math.max(20,Number(req.query.limit)||100));
  res.json({ok:true,counts:audit.counts,totalFiltered:rows.length,categories:[...new Set(audit.rows.map(x=>x.category))].sort(),rows:rows.slice(offset,offset+limit),offset,limit});
 }catch(e){res.status(500).json({ok:false,error:e.message})}
});
app.post('/api/admin/verification/run',requireAdmin,(req,res)=>{
 try{
  const qs=questions(),dir=path.join(DATA_DIR,'verification-backups');ensureDir(dir);const backup=path.join(dir,backupName());fs.copyFileSync(QUESTIONS_FILE,backup);
  const audit=auditBank(qs,{mutate:true,quarantineStructural:true});writeJsonAtomic(QUESTIONS_FILE,qs);
  res.json({ok:true,backup:path.basename(backup),...audit.counts});
 }catch(e){res.status(500).json({ok:false,error:e.message})}
});
app.post('/api/admin/verification/:id',requireAdmin,(req,res)=>{
 try{
  const qs=questions(),item=qs.find(x=>x.id===req.params.id);if(!item)return res.status(404).json({ok:false,error:'Frågan finns inte.'});
  const action=safe(req.body?.action),source=safe(req.body?.source);
  if(action==='quarantine'){item.quarantined=true;item.quarantineReason=safe(req.body?.reason)||'Manuell karantän';item.quarantinedAt=new Date().toISOString()}
  else if(action==='restore'){item.quarantined=false;delete item.verificationStatus;delete item.quarantineReason;delete item.quarantinedAt}
  else if(action==='verify-source'){if(!source)return res.status(400).json({ok:false,error:'Källa krävs för källverifiering.'});item.verified=true;item.verifiedAt=new Date().toISOString();item.source=source;item.verificationLevel='source-backed';item.verificationMethod='Admin manual source review';item.quarantined=false;delete item.verificationStatus}
  else if(action==='needs-review'){item.verified=false;item.verificationLevel='';item.verificationMethod='';item.verificationStatus='needs-review'}
  else return res.status(400).json({ok:false,error:'Okänd verifieringsåtgärd.'});
  item.verificationStatus=verificationStatus(item);writeJsonAtomic(QUESTIONS_FILE,qs);res.json({ok:true,question:item,status:item.verificationStatus});
 }catch(e){res.status(500).json({ok:false,error:e.message})}
});
app.get('/api/admin/verification.csv',requireAdmin,(req,res)=>{
 const a=auditBank(questions()),escCsv=v=>'"'+String(v??'').replace(/"/g,'""')+'"';
 const lines=['id,category,status,verified,duplicateText,issues,factKey,question,source',...a.rows.map(x=>[x.id,x.category,x.status,x.verified,x.duplicateText,x.issues.join('|'),x.factKey,x.question,x.source].map(escCsv).join(','))];
 res.set({'Content-Type':'text/csv; charset=utf-8','Content-Disposition':'attachment; filename="question-verification-audit.csv"','Cache-Control':'no-store'}).send('\ufeff'+lines.join('\n'));
});
...
io.on('connection',socket=>{
 ...
 socket.on('room:pause',(payload={},ack=()=>{})=>{const r=rooms.get(String(payload.code||''));if(!r||r.hostId!==socket.id)return ack({ok:false,error:'Endast värden kan pausa'});const pause=payload.paused!==false;if(pause&&r.phase==='question'&&!r.paused){r.remainingMs=Math.max(1000,r.deadline-Date.now());clearTimeout(r.timer);r.paused=true}else if(!pause&&r.phase==='question'&&r.paused){r.paused=false;r.deadline=Date.now()+(r.remainingMs||r.seconds*1000);r.timer=setTimeout(()=>reveal(r),r.remainingMs||r.seconds*1000);io.to(r.code).emit('room:question',{room:publicRoom(r),question:publicQuestion(r.quiz[r.questionIndex]),deadline:r.deadline,resume:true})}else r.paused=pause;ack({ok:true,room:publicRoom(r)});io.to(r.code).emit('room:update',publicRoom(r))});
 socket.on('room:answer-lock',(payload={},ack=()=>{})=>{const r=rooms.get(String(payload.code||''));if(!r||r.hostId!==socket.id)return ack({ok:false,error:'Endast värden kan låsa svar'});r.answerLocked=payload.locked!==false;ack({ok:true,room:publicRoom(r)});io.to(r.code).emit('room:update',publicRoom(r))});
 socket.on('room:bonus',(payload={},ack=()=>{})=>{const r=rooms.get(String(payload.code||''));if(!r||r.hostId!==socket.id)return ack({ok:false,error:'Endast värden kan välja bonus'});r.bonusNext=payload.enabled!==false;ack({ok:true,room:publicRoom(r)});io.to(r.code).emit('room:update',publicRoom(r))});
 socket.on('room:skip',(payload={},ack=()=>{})=>{const r=rooms.get(String(payload.code||''));if(!r||r.hostId!==socket.id)return ack({ok:false,error:'Endast värden kan hoppa över'});clearTimeout(r.timer);ack({ok:true});nextQuestion(r)});
 socket.on('room:team',(payload={},ack=()=>{})=>{const r=rooms.get(String(payload.code||'')),p=r?.players.get(socket.id);if(!r||!p)return ack({ok:false,error:'Rummet finns inte'});p.team=safe(payload.team).slice(0,20);ack({ok:true,room:publicRoom(r)});io.to(r.code).emit('room:update',publicRoom(r))});
