...
const QUESTION_METRICS_FILE=path.join(DATA_DIR,'question-metrics.json','question-ratings.json','seasons.json');
...
function smartPick(pool,count,r){
 const metrics=questionMetrics(),now=Date.now(),day=86400000;
 const recentPenalty=q=>{const t=Date.parse(metrics[q.id]?.lastPlayedAt||'');return Number.isFinite(t)&&now-t<7*day?18:0};
 const safe=pool.filter(q=>!metrics[q.id]||metrics[q.id].plays<5||questionQuality(q,metrics)>=30),source=safe.length>=count?safe:pool;
 const byCat=new Map(),tokens=x=>new Set(String(x||'').toLowerCase().replace(/[^a-zåäö0-9 ]/g,' ').split(/\s+/).filter(w=>w.length>4));
 for(const q of source){if(!byCat.has(q.c))byCat.set(q.c,[]);byCat.get(q.c).push({q,quality:questionQuality(q,metrics)-recentPenalty(q),rand:Math.random(),tok:tokens(q.q)})}
 for(const arr of byCat.values())arr.sort((a,b)=>(b.quality+b.rand*12)-(a.quality+a.rand*12));
 const cats=shuffle([...byCat.keys()]),chosen=[],chosenTokens=[];let cursor=0,lastCat='';
 const tooSimilar=x=>chosenTokens.slice(-5).some(t=>{let n=0;for(const w of x.tok)if(t.has(w))n++;return n>=3});
 while(chosen.length<count&&cats.length){
   ...
   const c=cats[found],x=byCat.get(c).shift();chosen.push(x.q);chosenTokens.push(x.tok);lastCat=c;cursor=(found+1)%cats.length;
...
function adminAllowed(req){
 ...
 const ip=String(req.ip||req.socket.remoteAddress||'');return ip.includes('127.3.0.1')||ip.includes('::1');
}
...
app.get('/health',(req,res)=>res.json({ok:true,version:'8.0.0',rooms:rooms.size,questions:allQuestions().length,categories:categoryCounts(),packs:allPacks().length}));
app.get('/api/questions/meta',(req,res)=>res.json({version:'8.0.0',questions:allQuestions().length,categories:categoryCounts(),difficulties:['easy','medium','hard'],packs:allPacks()}));
...
app.post('/api/question-rating',rateLimit('rating',60,60*1000),(req,res)=>{const id=cleanId(req.body?.questionId),vote=String(req.body?.vote||'');if(!id||!['up','down'].includes(vote))return res.status(400).json({ok:false});const x=readJson(QUESTION_RATINGS_FILE,{}),q=x[id]||(x[id]={up:0,down:0,lastAt:null});q[vote]++;q.lastAt=new Date().toISOString();writeJson(QUESTION_RATINGS_FILE,x);res.json({ok:true,rating:q});});
app.get('/api/diagnostics',(req,res)=>res.json({ok:true,version:'8.0.0',uptimeSeconds:Math.round(process.uptime()),rooms:rooms.size,questions:allQuestions().length,memoryMB:Math.round(process.memoryUsage().rss/1024/1024),dataDir:DATA_DIR,writable:(()=>{try{ensureDataDir();fs.accessSync(DATA_DIR,fs.constants.W_OK);return true}catch{return false}})()}));
...
// Admin/editor
app.get('/api/admin/status',(req,res)=>res.json({keyRequired:!!ADMIN_KEY,remoteAllowed:!!ADMIN_KEY,version:'8.0.0'}));
...
app.get('/api/admin/dashboard',requireAdmin,(req,res)=>{const games=readJson(GAMES_FILE,[]),metrics=questionMetrics(),ratings=readJson(QUESTION_RATINGS_FILE,{}),qs=allQuestions();const items=qs.map(q=>({id:q.id,q:q.q,c:q.c,quality:questionQuality(q,metrics),plays:metrics[q.id]?.plays||0,accuracy:metrics[q.id]?.total?pct(metrics[q.id].correct,metrics[q.id].total):null}));res.json({version:'8.0.0',questions:qs.length,categories:Object.keys(categoryCounts()).length,games:games.length,profiles:hallOfFame().players,rooms:rooms.size,review:items.filter(x=>x.plays>=3&&x.quality<45).length,ratings:Object.values(ratings).reduce((a,x)=>a+(x.up||0)+(x.down||0),0),tooEasy:items.filter(x=>x.plays>=3&&x.accuracy>=90).length,tooHard:items.filter(x=>x.plays>=3&&x.accuracy<=20).length,topReview:items.filter(x=>x.plays>=3).sort((a,b)=>a.quality-b.quality).slice(0,12)});});
app.get('/api/admin/backup',requireAdmin,(req,res)=>{ensureDataDir();const files=[HIGHSCORE_FILE,CUSTOM_FILE,PACK_FILE,OVERRIDE_FILE,GAMES_FILE,LEAGUE_FILE,PLAN_FILE,TOURNAMENT_FILE,TITLES_FILE,QUESTION_METRICS_FILE,QUESTION_RATINGS_FILE,SEASONS_FILE];const data={format:'resequiz-backup',version:1,appVersion:'8.0.0',createdAt:new Date().toISOString(),files:{}};for(const f of files)if(fs.existsSync(f))data.files[path.basename(f)]=readJson(f,null);res.setHeader('Content-Disposition',`attachment; filename=resequiz-backup-${new Date().toISOString().slice(0,10)}.json`);res.json(data)});
app.post('/api/admin/restore',rateLimit('restore',5,10*60*1000),requireAdmin,(req,res)=>{const b=req.body;if(!b||b.format!=='resequiz-backup'||!b.files||typeof b.files!=='object')return res.status(400).json({ok:false,error:'Ogiltig Resequiz-backup.'});const allowed=new Set(['highscores.json','custom-questions.json','custom-packs.json','question-overrides.json','games.json','league.json','game-plans.json','tournaments.json','player-titles.json','question-metrics.json','question-ratings.json','seasons.json']);let restored=0;for(const [name,data] of Object.entries(b.files)){if(!allowed.has(name)||data===null)continue;writeJson(path.join(DATA_DIR,name),data);restored++}res.json({ok:true,restored})});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Resequiz 8.0.0 listening on ${PORT}`));
