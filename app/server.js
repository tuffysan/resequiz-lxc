...
function adminAuth(){const x=readJson(ADMIN_AUTH_FILE,null);return x&&x.version===1&&x.username&&x.salt&&x.hash?x:null}
...
function passwordHash(password,salt){return crypto.scryptSync(String(password),Buffer.from(salt,'hex'),64,{N:16384,r:8,p:1}).toString('hex')}
function validPassword(password,auth){try{return safeEqualText(passwordHash(password,auth.salt),auth.hash)}catch{return false}}
...
app.get('/health',(req,res)=>res.json({ok:true,version:APP_VERSION,rooms:rooms.size,questions:allQuestions().length,categories:categoryCounts(),packs:allPacks().length}));
app.get('/api/questions/meta',(req,res)=>res.json({version:APP_VERSION,questions:allQuestions().length,categories:categoryCounts(),difficulties:['easy','medium','hard'],packs:allPacks()}));
...
app.post('/api/admin/setup',rateLimit('admin-setup',6,15*60*1000),(req,res)=>{if(adminAuth())return res.status(409).json({ok:false,error:'Administratörskontot är redan skapat.'});if(!ADMIN_KEY)return res.status(503).json({ok:false,error:'Adminnyckel är inte konfigurerad. Kör rotate-admin-key.sh i containern först.'});const supplied=String(req.body?.key||'').trim();if(!safeEqualText(supplied,ADMIN_KEY))return res.status(403).json({ok:false,error:'Fel adminnyckel.'});const username=clean(req.body?.username||'',60);const password=String(req.body?.password||'');if(!/^[A-Za-z0-9._@-]{3,60}$/.test(username))return res.status(400).json({ok:false,error:'Användarnamnet måste vara 3–60 tecken och får innehålla bokstäver, siffror, punkt, _, @ eller -.'});if(password.length<10||password.length>200)return res.status(400).json({ok:false,error:'Lösenordet måste vara minst 10 tecken.'});const salt=crypto.randomBytes(16).toString('hex'),auth={version:1,username,salt,hash:passwordHash(password,salt),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};writeJson(ADMIN_AUTH_FILE,auth);const t=newAdminSession(username);setAdminCookie(req,res,t);res.json({ok:true,username,version:APP_VERSION})});
app.post('/api/admin/login',rateLimit('admin-login',8,10*60*1000),(req,res)=>{const auth=adminAuth();if(!auth)return res.status(428).json({ok:false,error:'Administratörskontot är inte skapat ännu.',setupRequired:true});const username=String(req.body?.username||'').trim(),password=String(req.body?.password||'');if(!safeEqualText(username.toLowerCase(),String(auth.username).toLowerCase())||!validPassword(password,auth))return res.status(403).json({ok:false,error:'Fel användarnamn eller lösenord.'});const t=newAdminSession(auth.username);setAdminCookie(req,res,t);res.json({ok:true,username:auth.username,version:APP_VERSION})});
app.post('/api/admin/recover',rateLimit('admin-recover',4,30*60*1000),(req,res)=>{if(!ADMIN_KEY)return res.status(503).json({ok:false,error:'Adminnyckel är inte konfigurerad.'});const supplied=String(req.body?.key||'').trim();if(!safeEqualText(supplied,ADMIN_KEY))return res.status(403).json({ok:false,error:'Fel adminnyckel.'});const username=clean(req.body?.username||'',60),password=String(req.body?.password||'');if(!/^[A-Za-z0-9._@-]{3,60}$/.test(username)||password.length<10)return res.status(400).json({ok:false,error:'Ange giltigt användarnamn och ett lösenord på minst 10 tecken.'});const salt=crypto.randomBytes(16).toString('hex');writeJson(ADMIN_AUTH_FILE,{version:1,username,salt,hash:passwordHash(password,salt),createdAt:adminAuth()?.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()});adminSessions.clear();const t=newAdminSession(username);setAdminCookie(req,res,t);res.json({ok:true,username,version:APP_VERSION})});
app.post('/api/admin/change-password',requireAdmin,rateLimit('admin-password',5,15*60*1000),(req,res)=>{const auth=adminAuth();if(!auth)return res.status(404).json({ok:false,error:'Administratörskonto saknas.'});const current=String(req.body?.currentPassword||''),password=String(req.body?.newPassword||'');if(!validPassword(current,auth))return res.status(403).json({ok:false,error:'Nuvarande lösenord är fel.'});if(password.length<10||password.length>200)return res.status(400).json({ok:false,error:'Det nya lösenordet måste vara minst 10 tecken.'});const salt=crypto.randomBytes(16).toString('hex');writeJson(ADMIN_AUTH_FILE,{...auth,salt,hash:passwordHash(password,salt),updatedAt:new Date().toISOString()});adminSessions.clear();const t=newAdminSession(auth.username);setAdminCookie(req,res,t);res.json({ok:true})});
