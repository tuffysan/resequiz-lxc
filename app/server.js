...
function persistGame(r){
 ...
 writeHistory(h);persistDetailedGame(r,gameId);
}
...
function questionTypeFor(r,q){
 ...
 if(q._forcedType)return q._forcedType;
...
function dramaMoment(r,results,correctCount){
 ...
 let d=null;if(correctCount===0)d={icon:'💀',title:'Ingen klarade den!',text:'Brutal fråga.'};else if(correctCount===1){const one=results.find(x=>x.correct),p=findPlayer(r,one?.sessionId);d={icon:'🎯',title:'Ensam om rätt svar!',text:p?`${p.name} var den enda som klarade frågan.`:''}}else if(pctRight===100)d={icon:'🧠',title:'Full pott!',text:'Alla svarade rätt.'};else if(lead&&second&&lead.score-second.score<=100)d={icon:'⚔️',title:'Det är extremt jämnt!',text:`Bara ${lead.score-second.score} poäng skiljer ettan och tvåan.`};else{const hot=rank.find(p=>p.stats.streak>=5);if(hot)d={icon:'🔥',title:`${hot.stats.streak} rätt i rad!`,text:`${hot.name} är glödhet.`}}
 if(d){d.at=Date.now();r.events.push({at:d.at,type:'drama',...d});io.to(r.code).emit('drama',d)}return d
}
function directorSchedule(r,phase){if(!r.settings?.director)return;const old=directorTimers.get(r.code);if(old)clearTimeout(old);const ms=phase==='result'?5500:phase==='roundBreak'?7000:0;if(!ms)return;directorTimers.set(r.code,setTimeout(()=>{directorTimers.delete(r.code);if(phase==='result'&&r.phase==='result')advance(r);else if(phase==='roundBreak'&&r.phase==='roundBreak')startRoundQuestion(r)},ms))}
function settleQuestion(r,{skipped=false}={}){
 ...
 for(const p of r.players){
   ...
   results.push({sessionId:p.sessionId,correct:ok,gain,responseMs:ans?.responseMs??null});
 }
 if(r.currentType==='jackpot'&&correctCount>0)r.jackpot=0;const actualPct=pct(correctCount,Math.max(1,r.currentType==='duel'?r.duelists.length:r.players.length));for(const p of r.players){const pred=r.predictions?.get(p.sessionId);if(Number.isFinite(pred)&&Math.abs(pred-actualPct)<=15){p.score+=50;const rr=results.find(x=>x.sessionId===p.sessionId);if(rr){rr.predictionBonus=50;rr.gain+=50}}}
 ...
 r.lastResult={correctAnswer,explanation:q.f,results,room:roomPublic(r),hostSessionId:r.hostSessionId,skipped,secretBonus};
...
function finishGame(r){
 ...
 io.to(r.code).emit('gameOver',r.lastGameOver);
}
...
function adminAllowed(req){
 ...
 const ip=String(req.ip||req.socket.remoteAddress||'');return ip.includes('127.0.0.1')||ip.includes('::1');
}
...
app.get('/health',(req,res)=>res.json({ok:true,version:'5.2.0',rooms:rooms.size,questions:allQuestions().length,categories:categoryCounts(),packs:allPacks().length}));
app.get('/api/questions/meta',(req,res)=>res.json({version:'5.2.0',questions:allQuestions().length,categories:categoryCounts(),difficulties:['easy','medium','hard'],packs:allPacks()}));
...
// Admin/editor
app.get('/api/admin/status',(req,res)=>res.json({keyRequired:!!ADMIN_KEY,remoteAllowed:!!ADMIN_KEY}));
...
app.get('/api/admin/questions',requireAdmin,(req,res)=>res.json(readJson(CUSTOM_FILE,[])));
...
io.on('connection',socket=>{
 socket.on('createRoom',(d,cb=()=>{})=>{const name=clean(d?.name,20),sid=cleanId(d?.sessionId),avatar=cleanAvatar(d?.avatar);if(!name||!sid)return cb({ok:false,error:'Namn saknas.'});const r=makeRoom(name,sid);r.players[0].avatar=avatar;r.players[0].socketId=socket.id;socket.join(r.code);cb({ok:true,code:r.code,hostToken:r.hostToken,room:roomPublic(r)});emitRoom(r)});
 socket.on('joinRoom',(d,cb=()=>{})=>{const r=rooms.get(String(d?.code||'')),name=clean(d?.name,20),sid=cleanId(d?.sessionId),avatar=cleanAvatar(d?.avatar);if(!r)return cb({ok:false,error:'Rummet finns inte.'});if(r.phase!=='lobby')return cb({ok:false,error:'Spelet har redan startat.'});if(r.players.length>=100)return cb({ok:false,error:'Rummet är fullt (max 100).'});let p=findPlayer(r,sid);if(!p){p={sessionId:sid,name,avatar,team:clean(d?.team,20),score:0,connected:true,socketId:socket.id,stats:freshStats(),roundScores:[],powerups:{fifty:1,double:1,shield:1}};r.players.push(p)}else{p.name=name;p.avatar=avatar;p.connected=true;p.socketId=socket.id}socket.join(r.code);cb({ok:true,room:roomPublic(r)});emitRoom(r)});
 ...
 socket.on('startGame',(d,cb=()=>{})=>{const r=rooms.get(String(d?.code||''));if(!r||d.hostToken!==r.hostToken)return cb({ok:false,error:'Endast värden kan starta.'});if(r.players.length<2)return cb({ok:false,error:'Minst två deltagare krävs.'});const s=d.settings||{};r.mode=s.mode==='teams'?'teams':'individual';r.teamNames=Array.isArray(s.teamNames)?s.teamNames.map(x=>clean(x,20)).filter(Boolean).slice(0,4):r.teamNames;if(r.mode==='teams'&&s.autoTeams===false&&new Set(r.players.map(p=>p.team).filter(Boolean)).size<2)return cb({ok:false,error:'Minst två lag med deltagare krävs.'});const selectedPlan=allGamePlans().find(x=>x.id===cleanId(s.gamePlan));const family=selectedPlan?.family||s.profile==='family';r.settings={count:Math.min(200,Math.max(5,+s.count||20)),timer:family?Math.max(30,+s.timer||30):([0,10,15,20,30,45,60].includes(+s.timer)?+s.timer:15),difficulty:family?'easy':(['easy','medium','hard','mixed'].includes(s.difficulty)?s.difficulty:'mixed'),categories:Array.isArray(s.categories)?s.categories:[],roundSize:Math.min(20,Math.max(1,+s.roundSize||5)),finalBonusCount:family?0:Math.min(10,Math.max(0,+s.finalBonusCount||5)),pack:cleanId(s.pack),screenMode:s.screenMode==='display'?'display':'all',gamePlan:cleanId(s.gamePlan),profile:family?'family':(s.profile==='classic'?'classic':'ultimate'),powerups:s.powerups!==false,teamScoring:s.teamScoring==='sum'?'sum':'average',tournament:cleanId(s.tournament),smartDifficulty:s.smartDifficulty!==false,director:s.director!==false,drama:s.drama!==false,autoTeams:s.autoTeams!==false};r.deck=selectDeck(r,r.settings);if(r.deck.length<2)return cb({ok:false,error:'För få nya frågor i urvalet.'});r.deck.forEach(q=>r.seen.add(q.id));r.index=0;r.round=1;r.rounds=(selectedPlan?.rounds?.length)||Math.ceil(r.deck.length/r.settings.roundSize);r.questionStats=[];r.tiebreak=false;r.secretBonusIndex=family?-1:Math.floor(Math.random()*r.deck.length);if(r.mode==='teams'&&r.settings.autoTeams){const ps=shuffle(r.players),names=r.teamNames.slice(0,Math.min(4,Math.max(2,r.teamNames.length)));ps.forEach((p,i)=>p.team=names[i%names.length])}r.players.forEach(p=>{p.score=0;p.stats=freshStats();p.roundScores=[];p.powerups={fifty:1,double:1,shield:1};p.powerState={};p.captain=false});if(r.mode==='teams')for(const n of r.teamNames){const m=r.players.filter(p=>p.team===n);if(m[0])m[0].captain=true}r.persisted=false;cb({ok:true});startRoundQuestion(r)});
 ...
 socket.on('hostAction',(d,cb=()=>{})=>{const r=rooms.get(String(d?.code||''));if(!r||d.hostToken!==r.hostToken)return cb({ok:false,error:'Endast värden.'});const action=d.action;
   ...
   if(action==='randomTeams'&&r.mode==='teams'){const ps=shuffle(r.players);ps.forEach((p,i)=>p.team=r.teamNames[i%r.teamNames.length]);emitRoom(r);return cb({ok:true})}
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Resequiz 5.2 listening on ${PORT}`));
