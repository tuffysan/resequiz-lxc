...
const VERSION='19.6.1',PORT=Number(process.env.PORT||3000),HOST=process.env.HOST||'0.0.0.0';
...
function saveResult(candidate){const all=results();candidate.awards=awardsForResult(candidate,all);all.push(candidate);writeJsonAtomic(RESULTS_FILE,all.slice(-10000));return candidate}
...
app.get('/api/offline-pack',(req,res)=>{const count=Math.min(600,Math.max(100,Number(req.query.count)||350));res.json({version:VERSION,generatedAt:new Date().toISOString(),questions:shuffle(questions()).slice(0,count).map(offlineQuestion),childQuestions:childQuestions().map(offlineQuestion)})});
