...
app.get('/health',(req,res)=>res.json({ok:true,version:'9.0.0',rooms:rooms.size,questions:allQuestions().length,categories:categoryCounts(),packs:allPacks().length}));
app.get('/api/questions/meta',(req,res)=>res.json({version:'9.0.0',questions:allQuestions().length,categories:categoryCounts(),difficulties:['easy','medium','hard'],packs:allPacks()}));
...
app.get('/api/diagnostics',(req,res)=>res.json({ok:true,version:'9.0.0',uptimeSeconds:Math.round(process.uptime()),rooms:rooms.size,questions:allQuestions().length,memoryMB:Math.round(process.memoryUsage().rss/1024/1024),dataDir:DATA_DIR,writable:(()=>{try{ensureDataDir();fs.accessSync(DATA_DIR,fs.constants.W_OK);return true}catch{return false}})()}));
...
// Admin/editor
app.get('/api/admin/status',(req,res)=>res.json({keyRequired:!!ADMIN_KEY,remoteAllowed:!!ADMIN_KEY,version:'9.0.0'}));
...
server.listen(PORT,'0.0.0.0',()=>console.log(`Resequiz 9.0.0 listening on ${PORT}`));
