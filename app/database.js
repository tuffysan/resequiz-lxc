...
function openQuizDb(dataDir){
 ...
 CREATE TABLE IF NOT EXISTS question_metrics(question_id TEXT PRIMARY KEY,times_shown INTEGER NOT NULL DEFAULT 0,times_correct INTEGER NOT NULL DEFAULT 0,reported INTEGER NOT NULL DEFAULT 0,total_response_ms INTEGER NOT NULL DEFAULT 0,last_seen TEXT,answer_0 INTEGER NOT NULL DEFAULT 0,answer_1 INTEGER NOT NULL DEFAULT 0,answer_2 INTEGER NOT NULL DEFAULT 0,answer_3 INTEGER NOT NULL DEFAULT 0,answer_4 INTEGER NOT NULL DEFAULT 0,answer_5 INTEGER NOT NULL DEFAULT 0);
 ...
 for(let i=0;i<6;i++)if(!qm.includes('answer_'+i))try{db.exec(`ALTER TABLE question_metrics ADD COLUMN answer_${i} INTEGER NOT NULL DEFAULT 0`)}catch{}
 const row=db.prepare('SELECT COUNT(*) n FROM schema_info').get();
 if(!row.n)db.prepare('INSERT INTO schema_info(version) VALUES(?)').run(221);else db.prepare('UPDATE schema_info SET version=?').run(221);
 return db;
}
