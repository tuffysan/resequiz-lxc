...
if(!serverQuiet.includes('audioQuestions:s.audioQuestions===true')||!serverQuiet.includes("allQuestions().filter(q=>(s.audioQuestions===true||!q.audio)&&!globalSeen.has(q.id))")) throw new Error('Server audio filtering missing');
...
for(const marker of ['offline – återansluter','hostMigration','RESEQUIZ 12.0'])if(!online.includes(marker))throw new Error('10.0 client feature missing '+marker);
